...
vec2 u_resolution = vec2(800., 100.); // This should be passed in as a uniform but shader studio doesn't support that yet.
...
void main() {
  ...
  gl_FragColor = vec4(st.y / 7.,st.x,1. - st.x * st.y / 3.,1.); // R,G,B,A
